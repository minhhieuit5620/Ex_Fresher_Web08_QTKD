            ...
            <div class="login-form validate-form">
              ...
              <div class="wrap-input pass-wrap validate-input">
                <input
                  ...
                  @input="validateEmptyPass();errorMinLength=false"
                  placeholder-key="FormLogin_PasswordPlaceholder"
                  placeholder="Mật khẩu"
                  :class="{'border-red': emptyPassWord}" 
                  @click="emptyPassWord=false;errorMinLength=false"               
                  ref="password"
              ...
              <div
                ...
                style="display: none"
              >
                <div class="wrap-input captcha-wrap validate-input">
                  <div class="captcha-box">
                    ...
                    <i
                      ...
                      class="btn-captcha-reload"
                      title-key="FormLogin_CaptchaReloadTitle"
                      title="Lấy mã khác"
                    ></i>
                  </div>
                  <span class="error-info" res-key="FormLogin_CaptchaNotEmpty"
                    >Mã xác nhận không được để trống</span
                  >
                </div>
              </div>
              <div
...
import urlApi from '@/js/common/urlApi';
  ...
  // this.showButtonLoading=function(){
  // n.jBtnNext.addClass("loading");
  // n.jBtnNext.text(n.resource.FormOTP_ButtonLoading)};
  // this.removeButtonLoading=function(){n.jBtnNext.removeClass("loading")};
    ...
    */
    ...
    async login(username,password){
      try {       
        ...
        if(this.emptyPassWord===false && this.emptyUserName===false&&this.errorMinLength===false){
          var method=Resource.method.POST;
          let url=urlApi.apiAuth.login;
          try {
            var response = await baseApi.fetchAPILogin({username,password},url,method)  ;// get status
            let res=await response.json();//get data                
            let status= response.status;
            var token=res.data.accessToken;  
            var refreshToken=res.data.refreshToken;  
            var role=res.data.rol;
            var username=res.data.userName;                    
            let success=res.success;
            if(success){
              this.$router.push('/employeeList');
              localStorage.setItem(Resource.Manager.token.accessToken,token);
              localStorage.setItem(Resource.Manager.token.refreshToken,refreshToken);
              localStorage.setItem(Resource.Manager.token.rol,role);
              localStorage.setItem(Resource.Manager.token.username,username);
            } else{          
              if(status===eNum.errorBackEnd.Login|| status===eNum.errorBackEnd.Forbidden){             
                this.loginFailed=true;
              }
              else if(status===eNum.errorBackEnd.User){
...
<style scoped>
  /* .icon-vi {
  background-image: url(../images/icons/icon-vi.svg);
}
.icon-en {
  background-image: url(../images/icons/icon-en.svg);
}
.icon-authen-secure {
